...
import {
  ...
  isValidatedStatus,
  normalizeLibelle,
  resolveDisplayCompte,
  sortByIndex,
} from "@/src/features/bank/model/bank.model";
import { useBankStatementDetailViewModel } from "@/src/features/bank/viewmodel/useBankStatementDetailViewModel";
...
export function BankStatementDetailModal({
  ...
  const {
    ...
    handleSaveAll,
  } = useBankStatementDetailViewModel({
    open,
    statement,
    onUpdateTransaction,
  });
  ...
  const renderEditableCell = (
    ...
    if (readOnly) {
      return <div className={cn("rounded px-2 py-1", options?.className)}>{displayValue}</div>;
    }
