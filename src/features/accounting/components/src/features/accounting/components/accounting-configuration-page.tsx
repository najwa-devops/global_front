...
export function AccountingConfigurationPage({
  embedded = false,
}: AccountingConfigurationPageProps) {
  ...
  const [editingConfig, setEditingConfig] =
    useState<AccountingConfigRow | null>(null);
    ...
    const loadConfigs = async () => {
      try {
        ...
        setAccountingConfigs(
          Array.isArray(rows) ? (rows as AccountingConfigRow[]) : [],
        );
      } catch (error) {
  ...
  async function handleSave() {
    ...
    try {
      if (editingConfig) {
        const updated = await api.updateAccountingConfig(
          editingConfig.id,
          form,
        );
        setAccountingConfigs((prev) =>
          prev.map((row) =>
            row.id === editingConfig.id
              ? (updated as AccountingConfigRow)
              : row,
          ),
        );
        ...
        setAccountingConfigs((prev) => [
          created as AccountingConfigRow,
          ...prev,
        ]);
          ...
          <div className="grid gap-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                value={form.journal}
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, journal: e.target.value }))
                }
            ...
            <div className="flex items-center gap-2 rounded-md border p-3">
              ...
              />
              <Label htmlFor="ttcEnabled" className="cursor-pointer">
                TTC
              </Label>
            </div>
          </div>
