 ...
 */
export function extractFieldPosition(
  field: any,
 ...
 */
export function hasFieldsWithPositions(
...
export function dynamicInvoiceDtoToLocal(
  dto: DynamicInvoiceDto,
): DynamicInvoice {
  ...
  const technicalFields = [
    ...
    "status",
    "footerExtractionForced",
    "footerExtractionDate",
    "footerFieldsFound",
    "iceSource",
    "iceAddedDate",
    "iceAddedBy",
  ...
  const dossierRef =
    dto.dossierId !== undefined ? { dossierId: dto.dossierId } : {};
  ...
  return {
    ...
    autoFilledFields:
      dto.autoFilledFields || dto.fieldsData?.autoFilledFields || [],
    ...
    canCreateTemplate:
      dto.canCreateTemplate || dto.fieldsData?.canCreateTemplate || false,
    ...
    templateDetected:
      dto.fieldsData?.templateDetected || dto.templateDetected || false,
    overallConfidence:
      dto.fieldsData?.overallConfidence || dto.overallConfidence,
    averageConfidence:
      dto.fieldsData?.averageConfidence || dto.averageConfidence,
    ...
    clientValidatedAt: dto.clientValidatedAt
      ? new Date(dto.clientValidatedAt)
      : undefined,
...
export function dynamicInvoiceToUpdateDto(
...
export function filterDynamicInvoices(
  invoices: DynamicInvoice[],
  searchTerm: string,
): DynamicInvoice[] {
  if (!searchTerm.trim()) {
...
export function estimateFieldPosition(
  ...
  if (!fieldValue || !ocrText || String(fieldValue).length < 2)
    return undefined;
...
export function ParseBackendFieldsData(
  ...
  if (fieldsData.fixedSupplierData) {
    ...
    if (
      supplierField &&
      (!supplierField.value || supplierField.value === "") &&
      fixed.supplier
    ) {
  ...
  if (
    ifField &&
    (!ifField.value || ifField.value === "") &&
    fieldsData.detectedIf
  ) {
