  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        auxiliaireMode:
          existingTier?.auxiliaireMode ??
          (typeof window !== "undefined"
            ? localStorage.getItem("accounting_global_aux_mode") === "true"
            : true),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.auxiliaireMode) {
        if (!formData.collectifAccount)
          throw new Error("Le compte collectif est requis en mode auxiliaire");
        if (!/^(342100000|441100000)$/.test(formData.collectifAccount)) {
          throw new Error(
            "Compte collectif invalide (doit être 342100000 ou 441100000)",
          );
        }
      ...
      if (
        formData.defaultChargeAccount &&
        !/^\d{9}$/.test(formData.defaultChargeAccount)
      ) {
        throw new Error(
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setFormData({ ...formData, defaultChargeAccount: v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Sélectionner..." />
