...
export function CreateTemplateModal({
    ...
    const [selectedSignature, setSelectedSignature] = useState<AvailableSignature | null>(null)
    ...
    const fetchSignatures = async () => {
        ...
        try {
            ...
            const recommended = data.signatures?.find((s: AvailableSignature) => s.recommended)
            if (recommended) {
            ...
            setError("Impossible de récupérer les signatures disponibles pour cette facture.")
        } finally {
            setIsLoading(false)
        }
    }
                ...
                <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 text-white">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-xl font-bold text-white">Créer un Template</DialogTitle>
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            {isLoading ? (
                                <div className="flex flex-col items-center justify-center py-12 space-y-4">
                                    ...
                                    <p className="text-sm text-slate-500 animate-pulse">Analyse des données OCR...</p>
                                </div>
                            ) : error ? (
                                <div className="p-4 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3">
                                    ...
                                    <div className="text-sm text-red-700">
                                        ...
                                        <p>{error}</p>
                                        <Button variant="link" size="sm" onClick={fetchSignatures} className="p-0 h-auto text-red-600 mt-1">Réessayer</Button>
                                    </div>
                                </div>
                            ) : signatures.length === 0 ? (
                                <div className="p-6 text-center border-2 border-dashed border-slate-200 rounded-xl space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        <p className="font-semibold text-slate-900">Aucune signature détectée</p>
                                        <p className="text-xs text-slate-500 max-w-[250px] mx-auto">
                                            L'OCR n'a pas trouvé d'IF, d'ICE ou de RC valide sur ce document.
                                        </p>
                                    </div>
                                </div>
                            ) : (
                                <div className="grid gap-3">
                                    {signatures.map((sig) => (
                                        <Card
                                            ...
                                            className={`cursor-pointer transition-all border-2 overflow-hidden ${selectedSignature?.type === sig.type
                                        ...
                                        >
                                            <CardContent className="p-4">
                                                <div className="flex items-center justify-between gap-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`p-2 rounded-full ${selectedSignature?.type === sig.type ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'
                                                            }`}>
                                                            <CheckCircle2 className="h-4 w-4" />
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                <span className="font-bold text-slate-900">{sig.type}</span>
                                                                {sig.recommended && (
                                                            ...
                                                            <p className="text-xs font-mono text-slate-600 mt-0.5">{sig.value}</p>
                                                        </div>
                                                    </div>
                                                    {sig.reason && (
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg flex items-start gap-3 border border-slate-200 dark:border-slate-800">
                            ...
                            <p className="text-[11px] text-slate-600 leading-relaxed">
                                <span className="font-bold text-blue-700">Pourquoi une signature ?</span> Elle permet au système de reconnaître instantanément ce fournisseur sur ses prochaines factures, quel que soit le format.
                            </p>
                        </div>
                    </div>
                    ...
                    <DialogFooter className="p-6 bg-slate-50/50 border-t border-slate-100 dark:border-slate-900 mt-0">
                        ...
                        <Button
                            ...
                            disabled={isSubmitting || isLoading || !selectedSignature || !templateName.trim()}
