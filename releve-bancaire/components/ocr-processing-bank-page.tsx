...
import type { LocalBankStatement, BankStatementField } from "@/releve-bancaire/types"
...
export function OcrProcessingBankPage({
    ...
    // Resynchronise les champs quand le parent met à jour le statement (après reprocessing)
    useEffect(() => {
        setFields(statement.fields)
    }, [statement.fields])
    ...
    // Déclenche la détection automatique (banque + structure) via le backend,
    // puis met à jour les champs affichés avec les données extraites.
    const handleOcrExtract = async () => {
        if (!onReprocess) return
        setIsProcessingOcr(true)
        toast.loading("Détection automatique en cours...")
        try {
            await onReprocess()
            toast.dismiss()
            toast.success("Banque détectée et données extraites")
        } catch (error) {
            toast.dismiss()
