...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const hasQueryDossierId =
      Number.isFinite(queryDossierId) && queryDossierId > 0;

    if (typeof window !== "undefined") {
      if (hasQueryDossierId) {
        localStorage.setItem("currentDossierId", String(queryDossierId));
      } else {
        const storedDossierId = Number(
          localStorage.getItem("currentDossierId"),
        );
        const hasStoredDossierId =
          Number.isFinite(storedDossierId) && storedDossierId > 0;
        if (!hasStoredDossierId) {
          toast.error(
            "Dossier requis: ouvrez un dossier avant d'acceder a la facture.",
          );
