...
import {
    ArrowLeft, Upload, FileText, Building2, CheckCircle2,
    Clock, Eye, Sparkles, Trash2, FolderOpen, AlertCircle
} from "lucide-react"
...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
    VALIDATED: { label: "Validée", color: "bg-green-500/10 text-green-600 border-green-500/20", icon: CheckCircle2 },
    READY_TO_VALIDATE: { label: "Prête", color: "bg-blue-500/10 text-blue-600 border-blue-500/20", icon: Sparkles },
    READY_TO_TREAT: { label: "En attente", color: "bg-amber-500/10 text-amber-600 border-amber-500/20", icon: Clock },
    VERIFY: { label: "À vérifier", color: "bg-indigo-500/10 text-indigo-600 border-indigo-500/20", icon: Clock },
    REJECTED: { label: "Rejetée", color: "bg-red-500/10 text-red-600 border-red-500/20", icon: AlertCircle },
}
...
function DossierDetailContent() {
    ...
    const loadData = async () => {
        try {
            ...
            const currentDossier = (allDossiers || []).find((d: any) => d.id === dossierId) || null
    ...
    const stats = useMemo(() => {
        ...
        const validated = invoices.filter((i) => toWorkflowStatus(i.status) === "VALIDATED").length
        return {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div>
                            <h2 className="text-xl font-bold">{dossier?.name || "..."}</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    { label: "Total factures", value: stats.total, color: "text-blue-500", bg: "bg-blue-500/10", icon: FileText },
                    { label: "En attente", value: stats.pending, color: "text-amber-500", bg: "bg-amber-500/10", icon: Clock },
                    { label: "Validées", value: stats.validated, color: "text-green-500", bg: "bg-green-500/10", icon: CheckCircle2 },
                    { label: "Relevés bancaires", value: stats.bank, color: "text-purple-500", bg: "bg-purple-500/10", icon: Building2 },
                ].map(stat => (
                ...
                <TabsContent value="invoices" className="mt-4">
                    {invoices.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p className="text-muted-foreground">Aucune facture dans ce dossier</p>
                                            ...
                                            <div className="flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-muted-foreground">{new Date(date).toLocaleDateString("fr-FR")}</p>
                                                    </div>
                                                </div>
                ...
                <TabsContent value="bank" className="mt-4">
                    {bankStatements.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p className="text-muted-foreground">Aucun relevé bancaire dans ce dossier</p>
                            {canEdit && (
                                <Button className="mt-4 gap-2" onClick={() => router.push("/bank/upload")}>
                                    <Upload className="h-4 w-4" />
                                    Importer un relevé
                                </Button>
                            )}
                                    ...
                                    <CardContent className="py-3 px-4">
                                        <div className="flex items-center justify-between gap-4">
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="font-medium text-sm truncate">{stmt.filename || stmt.originalName}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </CardContent>
